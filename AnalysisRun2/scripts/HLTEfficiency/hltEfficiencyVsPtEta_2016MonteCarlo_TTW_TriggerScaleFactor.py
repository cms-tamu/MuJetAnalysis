...
def efficiency_trigger(dirNames, triggerPaths):
    ...
    MyFile = TFile("HLT_Z_peak_signal_2016MonteCarlo_TTWJets_13TeV.root","RECREATE");
...
def makePlot(histogram, plotType, x_label, y_label, saveAs, format='pdf'):
    ...
    hist.saveDir     = 'trigger_efficiency_plots_2016MonteCarlo_TTWJets_20180220/'
    hist.saveAs      = saveAs# "Z_peak_2016MonteCarlo_TTW" # save figure with name
    ...
    hist.Add(histogram, draw='errorbar', color='black', linecolor='black', label=r'$TTW \rightarrow 3\mu\nu$')
    plot = hist.execute()
    hist.savefig()

MyFile = TFile("HLT_Z_peak_signal_2016MonteCarlo_TTWJets_13TeV.root")

Invariant_Mass12 = MyFile.Get("Invariant_Mass12")
makePlot(Invariant_Mass12, "histogram", 
         r"Dimuon invariant mass [GeV]", "Entries", "Z_peak_2016MonteCarlo_TTW", format='pdf')

Invariant_Mass123 = MyFile.Get("Invariant_Mass123")
makePlot(Invariant_Mass123, "histogram", 
         r"Trimuon invariant mass [GeV]", "Entries", "Trimuon_invariant_mass_2016MonteCarlo_TTW", format='pdf')

Transverse_Mass = MyFile.Get("Transverse_Mass")
makePlot(Transverse_Mass, "histogram", 
         r"Transverse mass m_\mathrm{T} [GeV]", "Entries", "Transverse_mass_2016MonteCarlo_TTW", format='pdf')

PFMET = MyFile.Get("PFMET")
makePlot(PFMET, "histogram", 
         r"PFMET [GeV]", "Entries", "PFMET_2016MonteCarlo_TTW", format='pdf')

eff_trig_leading_muon_pt = MyFile.Get("eff_trig_leading_muon_pt")
makePlot(eff_trig_leading_muon_pt, "efficiency", 
         r"Leading muon $p_\mathrm{T}$ [GeV]", "Trigger efficiency", "Trigger_efficiency_leading_pt_2016MonteCarlo_TTW", format='pdf')

eff_trig_leading_muon_eta = MyFile.Get("eff_trig_leading_muon_eta")
makePlot(eff_trig_leading_muon_eta, "efficiency", 
         r"Leading muon $\eta$", "Trigger efficiency", "Trigger_efficiency_leading_eta_2016MonteCarlo_TTW", format='pdf')

eff_trig_leading_muon_phi = MyFile.Get("eff_trig_leading_muon_phi")
makePlot(eff_trig_leading_muon_phi, "efficiency", 
         r"Leading muon $\phi$", "Trigger efficiency", "Trigger_efficiency_leading_phi_2016MonteCarlo_TTW", format='pdf')
