...
void efficiency_trigger(const std::vector<std::string>& dirNames, const std::string& triggerName)
{
  bool verbose(false);
  ...
  std::vector<std::string > * pPaths = 0;
  ...
  while ((chEl=(TChainElement*)next())) {
    ...
    t->SetBranchAddress("hltPaths",&pPaths);
  ...
  struct MyEfficiencyPlot
  {
    static void plot(enum Variable v, TEfficiency* eff, TString cTitle) { 
      ...
      int xBin = 50;
      TString xTitle;
      TString yTitle = triggerName + " efficiency";
      TString title;
