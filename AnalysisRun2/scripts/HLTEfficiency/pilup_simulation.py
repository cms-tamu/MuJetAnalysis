...
pileup_plot = TH1D("pileup_plot","",60,0,60)
...
def makePlot(histogram, plotType, x_label, y_label, saveAs, format='pdf'):
    ...
    hist.plotLUMI = False
    hist.extra_text.Add(r"$<\mu> = 25$", coords=[0.55,0.9])
    hist.drawStatUncertainty = False    
    hist.Add(histogram, draw='step', color='blue', linecolor='blue', label=r'Run-2 Simulation 2016')
    plot = hist.execute()
    hist.savefig()
...
for p, pp in zip(probFunctionVariable,probValue):
    pileup_plot.SetBinContent(p+1, pp)

print pileup_plot.GetMean()
makePlot(pileup_plot, "histogram", "Mean number of interactions per crossing", "Probability", "pileup_plot_2016", format='pdf')
