...
//eta v Lxy
//TH2F *Real_Reco_Eta_vs_Lxy = new TH2F("Real_Reco_Eta_vs_Lxy","Real_Reco_Eta_vs_Lxy",200, -5, 5, 320, -80, 80);
//TH2F *Real_Reco_Eta_vs_Lxy = new TH2F("Real_Reco_Eta_vs_Lxy","Real_Reco_Eta_vs_Lxy",200, -5, 5, 160, -80, 80);
//TH2F *Real_Reco_Eta_vs_Lxy = new TH2F("Real_Reco_Eta_vs_Lxy","Real_Reco_Eta_vs_Lxy",100, -5, 5, 320, -80, 80); //In Mathematica Script
//TH2F *Real_Reco_Eta_vs_Lxy = new TH2F("Real_Reco_Eta_vs_Lxy","Real_Reco_Eta_vs_Lxy",100, -5, 5, 800, -80, 80); //In Mathematica Script
TH2F *Real_Reco_Eta_vs_Lxy = new TH2F("Real_Reco_Eta_vs_Lxy","Real_Reco_Eta_vs_Lxy",1000, -5, 5, 16000, -80, 80); //In Mathematica Script
TH2F *Fake_Reco_Eta_vs_Lxy = new TH2F("Fake_Reco_Eta_vs_Lxy","Fake_Reco_Eta_vs_Lxy",100, -5, 5, 200, -80, 80);
	...
	while ((chEl=(TChainElement*)next())) {
		...
		for(int k=0;k<t->GetEntries();k++){
			...
			if(is4GenMu8){
				...
				//if(fabs(dphi_gD)>=2.5){
				if(genA0_Lxy<10.2 && genA1_Lxy<10.2 && fabs(genA0_Lz)<48.5 && fabs(genA1_Lz)<48.5){
					ev_4gmlxylzcut++;
				}
				//}
			}
			...
			//{{{
			if(is4GenMu8){
				if(genA0_Lxy<=11.0 && genA1_Lxy<=11.0 && fabs(genA0_Lz)<=48.5 && fabs(genA1_Lz)<=48.5){
				//if(genA0_Lxy<10.2 && genA1_Lxy<10.2 && fabs(genA0_Lz)<48.5 && fabs(genA1_Lz)<48.5)
					triggerFlag_lxy++;
				}
			}
			...
			//Calculating 97% levels in Eta vs Lxy.  In Eta bins.
/*
for(int i = 1; i < 500; i++){
				double integral = 0.0000000000001;
				for (int j = 1; j < 400; j++){
					integral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,j);
				}
				double runIntegral = 0;
				cout << "(integral, i): " << integral << ", " << i << endl;
				for (int k = 1; k < 400; k++){
					runIntegral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,k)/integral;
					if(runIntegral >= 0.97){
						cout << "eta = " << -5+(i-0.5)/50 << " 97% cut: " << double(k)/80.0 << endl;
						break;
					}
				}
			}
*/
			...
			if(triggerFlag_std > 0 && triggerFlag_lxy == 0){
				...
				if(genA0_Lxy>=11.0 || fabs(genA0_Lz)>=48.5 || genA1_Lxy>=11.0 || fabs(genA1_Lz)>=48.5 ){ //We only want to perform this loop on a very pure sample of fakes.  Set to 11.0 instead of 10.2. 
					//Now match 
					//cout << "" << endl;
					...
					if(MatchRECOtoGENdimuons(diMuonC_FittedVtx_eta, diMuonC_FittedVtx_phi, diMuonF_FittedVtx_eta, diMuonF_FittedVtx_phi, genA0_eta, genA0_phi, genA1_eta, genA0_phi) == 0){ //diMuonC -> A0, diMuonF -> A1
						if(genA0_Lxy>=11.0 || fabs(genA0_Lz)>=48.5){ //A0 is the fake and has match diMuonC
						...
						if(genA1_Lxy>=11.0 || fabs(genA1_Lz)>=48.5){ //A1 is the fake and has match diMuonF
							//NOW LOOK AT INDIVIDUAL MUONS
							//{{{                                        
						...
						if(genA0_Lxy>=11.0 || fabs(genA0_Lz)>=48.5){ //A0 is the fake and has match diMuonF
							//NOW LOOK AT INDIVIDUAL MUONS
							//{{{                                        
							//HERE, we know that DIMUONF is bad
							//DIMUONF contains RECO muons reco_real_muons_phi[diMuonF_match0] and reco_real_muons_phi[diMuonF_match1]
						...
						if(genA1_Lxy>=11.0 || fabs(genA1_Lz)>=48.5){ //A1 is the fake and has match diMuonC
							//NOW LOOK AT INDIVIDUAL MUONS
							//{{{
							//HERE, we know that DIMUONC is bad
							//DIMUONC contains RECO muons reco_real_muons_phi[diMuonC_match0] and reco_real_muons_phi[diMuonC_match1]
				...
				if(genA0_Lxy >= 10.2 && fabs(genA0_Lz < 48.5)) fakeCounter_dimuon1 = 1; //Fails only Lxy
				if(genA0_Lxy < 10.2 && fabs(genA0_Lz >= 48.5)) fakeCounter_dimuon1 = 2; //Fails only Lz
				if(genA0_Lxy >= 10.2 && fabs(genA0_Lz >= 48.5)) fakeCounter_dimuon1 = 3; //Fails both Lxy and Lz
				if(genA1_Lxy >= 10.2 && fabs(genA1_Lz < 48.5)) fakeCounter_dimuon2 = 4; //Fails only Lxy
				if(genA1_Lxy < 10.2 && fabs(genA1_Lz >= 48.5)) fakeCounter_dimuon2 = 5; //Fails only Lz
				if(genA1_Lxy >= 10.2 && fabs(genA1_Lz >= 48.5)) fakeCounter_dimuon2 = 6; //Fails both Lxy and Lz
...
for(int i = 1; i < 500; i++){
		...
		for (int k = 1; k < 400; k++){
			runIntegral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,k)/integral;
			if(runIntegral >= 0.97){
				cout << "eta = " << -5+(i-0.5)/50 << " 97% cut: " << double(k)/5 << endl;                                   
				break;
			}
		}
	} 
*/
	///
	for(int i = 0; i < 100; i++){ //# eta bins
		double integral = 0.0000000000001;
		for (int j = 0; j < 800; j++){ //# Lxy bins
			integral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,j);
		}       
		double runIntegral = 0;
		for (int k = 0; k < 800; k++){ //# Lxy bins
			runIntegral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,k)/integral;
			if(runIntegral >= 0.97){
				cout << "eta = " << -5+(double((i)/10.0)) << "\t 97% cut: \t" << (double(k)/(800.0/160.0) - 80) << " \t Percentage: \t" << runIntegral << endl;//#bins/Range(10 or 160) for denominator
				break;
			}
		}
	}
...
//Done with Alexei.  Frozen:
	/*
	   ...
	   runIntegral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,k)/integral;
	   if(runIntegral >= 0.97){
	   cout << "eta = " << -5+(i-0.5)/10 << " 97% cut: " << (double(k)/1.25 - 80) << endl;
	   break;
	   }
	   }
	   }
