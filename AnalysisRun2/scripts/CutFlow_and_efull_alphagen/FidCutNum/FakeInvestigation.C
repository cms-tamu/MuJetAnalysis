...
TFile *BAM = new TFile("erec_alphagen_FittedVTX_WithElipCut.root","RECREATE");
...
void efficiency_hitrecovery(TString fileName){
	...
	double best_Eta = 0;
	double best_Lxy = 0;
	//for(double eta_cut = 0; eta_cut < eta_max; eta_cut += eta_stepsize){
	//	for(double lxy_cut = 0; lxy_cut < lxy_max; lxy_cut += lxy_stepsize){
			double Signal = 0;
			double Background = 0;
...
cout << "mass, cT, erec: " << mass_double << ", " << cT_double << ", " << (double)ev_isDiMuonHLTFired/ev_all << endl; 
...
void makePlot(){
	...
	c->SaveAs("e_rec_alpha_gen_vs_cT_withElipCut.pdf","recreate");
}
