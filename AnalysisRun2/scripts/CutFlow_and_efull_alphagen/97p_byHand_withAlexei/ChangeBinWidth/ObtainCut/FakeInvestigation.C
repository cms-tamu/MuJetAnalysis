...
void efficiency_hitrecovery(TString fileName){
	...
	while ((chEl=(TChainElement*)next())) {
		...
		for(int k=0;k<t->GetEntries();k++){
			...
			//{{{
			if(is4GenMu8){
				if(genA0_Lxy<=5.0 && genA1_Lxy<=5.0 && fabs(genA0_Lz)<=34.5 && fabs(genA1_Lz)<=34.5){
				//if(genA0_Lxy<4.4 && genA1_Lxy<4.4 && fabs(genA0_Lz)<34.5 && fabs(genA1_Lz)<34.5)
					triggerFlag_lxy++;
				}
			}
...
*/
			...
			if(triggerFlag_std > 0 && triggerFlag_lxy == 0){
				...
				if(genA0_Lxy>=5.0 || fabs(genA0_Lz)>=34.5 || genA1_Lxy>=5.0 || fabs(genA1_Lz)>=34.5 ){ //We only want to perform this loop on a very pure sample of fakes.  Set to 5.0 instead of 4.4. 
					//Now match 
					//cout << "" << endl;
					...
					if(MatchRECOtoGENdimuons(diMuonC_FittedVtx_eta, diMuonC_FittedVtx_phi, diMuonF_FittedVtx_eta, diMuonF_FittedVtx_phi, genA0_eta, genA0_phi, genA1_eta, genA0_phi) == 0){ //diMuonC -> A0, diMuonF -> A1
						if(genA0_Lxy>=5.0 || fabs(genA0_Lz)>=34.5){ //A0 is the fake and has match diMuonC
						...
						if(genA1_Lxy>=5.0 || fabs(genA1_Lz)>=34.5){ //A1 is the fake and has match diMuonF
							//NOW LOOK AT INDIVIDUAL MUONS
							//{{{                                        
						...
						if(genA0_Lxy>=5.0 || fabs(genA0_Lz)>=34.5){ //A0 is the fake and has match diMuonF
							//NOW LOOK AT INDIVIDUAL MUONS
							//{{{                                        
							//HERE, we know that DIMUONF is bad
							//DIMUONF contains RECO muons reco_real_muons_phi[diMuonF_match0] and reco_real_muons_phi[diMuonF_match1]
						...
						if(genA1_Lxy>=5.0 || fabs(genA1_Lz)>=34.5){ //A1 is the fake and has match diMuonC
							//NOW LOOK AT INDIVIDUAL MUONS
							//{{{
							//HERE, we know that DIMUONC is bad
							//DIMUONC contains RECO muons reco_real_muons_phi[diMuonC_match0] and reco_real_muons_phi[diMuonC_match1]
				...
				if(genA0_Lxy >= 4.4 && fabs(genA0_Lz < 34.5)) fakeCounter_dimuon1 = 1; //Fails only Lxy
				if(genA0_Lxy < 4.4 && fabs(genA0_Lz >= 34.5)) fakeCounter_dimuon1 = 2; //Fails only Lz
				if(genA0_Lxy >= 4.4 && fabs(genA0_Lz >= 34.5)) fakeCounter_dimuon1 = 3; //Fails both Lxy and Lz
				if(genA1_Lxy >= 4.4 && fabs(genA1_Lz < 34.5)) fakeCounter_dimuon2 = 4; //Fails only Lxy
				if(genA1_Lxy < 4.4 && fabs(genA1_Lz >= 34.5)) fakeCounter_dimuon2 = 5; //Fails only Lz
				if(genA1_Lxy >= 4.4 && fabs(genA1_Lz >= 34.5)) fakeCounter_dimuon2 = 6; //Fails both Lxy and Lz
