...
//eta v Lxy
TH2F *Real_Reco_Eta_vs_Lxy = new TH2F("Real_Reco_Eta_vs_Lxy","Real_Reco_Eta_vs_Lxy",100, -5, 5, 200, -80, 80);
...
*/
	...
	for(int i = 0; i < 100; i++){
		...
		for (int k = 0; k < 200; k++){
			runIntegral += Real_Reco_Eta_vs_Lxy->GetBinContent(i,k)/integral;
			if(runIntegral >= 0.97){
				cout << "eta = " << -5+(double((i)/10.0)) << "\t 97% cut: \t" << (double(k)/1.25 - 80) << " \t Percentage: \t" << runIntegral << endl;                                   
				break;
			}
		}
	}
