...
void AnalyzePlots(TString filename){
	...
	for(double eta_cut = eta_min; eta_cut <= eta_max; eta_cut += eta_stepsize){
		for(double lxy_cut = lxy_min; lxy_cut <= lxy_max; lxy_cut += lxy_stepsize){
			...
			if(eta_graph->GetMean(2) > 0.95){ //2 will retrieve mean from y-axis
			...
			if(lxy_graph->GetMean(2) > 0.95){ //2 will retrieve mean from y-axis
			...
			if(isEtaFlat && isLxyFlat){
				cout << "Both efficiency plots are flat and have a mean > 95% for (Eta, Lxy) " << eta_cut << ", " << lxy_cut << endl;
			}
		}
	}
}
