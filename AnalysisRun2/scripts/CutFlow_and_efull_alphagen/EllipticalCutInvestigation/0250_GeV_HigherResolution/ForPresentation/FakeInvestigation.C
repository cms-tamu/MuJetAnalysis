...
void efficiency_hitrecovery(TString fileName){
	...
	double best_Eta = 0;
	double best_Lxy = 0;
	for(double eta_cut = 2.2; eta_cut < eta_max; eta_cut += eta_stepsize){
		for(double lxy_cut = 8; lxy_cut < lxy_max; lxy_cut += lxy_stepsize){
			double Signal = 0;
			double Background = 0;
...
*/
			cout << "In Progress (Eta, Lxy, Signal, Background, FOM): " << eta_cut << ", " << lxy_cut << ", " << Signal << ", " << Background << ", "  << FOM(Signal, Background) << endl;
