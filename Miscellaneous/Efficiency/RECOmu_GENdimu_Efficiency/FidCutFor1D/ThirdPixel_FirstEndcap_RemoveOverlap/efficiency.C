...
double eta(double r, double z){
	double value = -log( tan( atan2(r,z)/2  )  );
	return value;
}
			...
			if(isDiMuonHLTFired == 1){ //Make sure that the event fired the HLT
				...
				if(is4GenMu8==true){
					...
					if(fabs(genA0_Lz_rdet) <= 34.5){ //First pixel endcap
						...
						if(fabs(eta(genA0_Lxy_rdet, genA0_Lz_rdet)) <= 0.9 || fabs(eta(genA0_Lxy_rdet, genA0_Lz_rdet)) >= 1.5){
							for(int n=0;n<160;n++){
...
*/
					...
					if(fabs(genA1_Lz_rdet) <= 34.5){ //First pixel endcap
						if(genA1_Lxy_rdet <= 11.0){ //First tracker barrel
						 if(fabs(eta(genA1_Lxy_rdet, genA1_Lz_rdet)) <= 0.9 || fabs(eta(genA1_Lxy_rdet, genA1_Lz_rdet)) >= 1.5){ 
							for(int n=0;n<160;n++){
