...
void efficiency(const std::vector<std::string>& dirNames)
{
  ...
  Bool_t  is1DiMuonsFittedVtxOK;
  
  Bool_t  is2DiMuonHLTFired;
  Float_t diMuon_IsoTk_FittedVtx;
  ...
  TH1F *mass= new TH1F("mass","",200,0.0,1.0);

  TH1F *iso = new TH1F("iso","",200,0.0,10.0);
...
//   f[1] = new TFile("DarkSUSY_mH_125_mGammaD_0700_ctauExp_02_8TeV-madgraph452_bridge224_LHE_pythia6_5374_PAT_v1_10000.root");
	...
	// 5) allocate var to branch
		t->SetBranchAddress("diMuon_FittedVtx_m",&diMuon_Mass);
		...
		t->SetBranchAddress("diMuon_IsoTk_FittedVtx",&diMuon_IsoTk_FittedVtx);

		t->SetBranchAddress("is1DiMuons",&is1DiMuons);
		...
		for(int i=0;i<nentries;i++){ //loop for number of events
		  ...
		  if(nRecoMu>=4){
		    ...
		    if(is1DiMuons){
		      mass->Fill(mass);
		      iso->Fill(diMuon_IsoTk_FittedVtx);
		    }
		  }
