...
class Analysis2012 : public edm::EDAnalyzer {
public:
  explicit Analysis2012(const edm::ParameterSet&);
  ~Analysis2012();
...
//
Analysis2012::Analysis2012(const edm::ParameterSet& iConfig)
{
  param_ = iConfig;
...
void
Analysis2012::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
  ...
  if(is4recm){
	...
	//     // if(m_debug>10){
	//     //   if(k==0) std::cout<< "p.hasValidHitInPixelLayer(PixelSubdetector::PixelEndcap, 1): " << p.hasValidHitInPixelLayer(PixelSubdetector::PixelEndcap, 1) <<counter_gt<<std::endl;
	//     //   if(k==1) std::cout<< "  matching   gt  mu2 muJetC  "<<counter_gt<<std::endl;
	//     // }
...
void 
Analysis2012::beginJob() {
  // std::cout << "BEGIN JOB" << std::endl;
  
  // edm::Service<TFileService> tFileService;
  // TH1F *min_dm1m2 = tFileService->make<TH1F>( "min_dm1m2"  , "min_dm1m2", 100,  0.,1.0);
...
// ------------ method called once each job just before starting event loop  ------------
void 
Analysis2012::beginRun(edm::Run & run, const edm::EventSetup&)
{
}
...
DEFINE_FWK_MODULE(Analysis2012);
